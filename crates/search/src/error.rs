...
use search_types::document_formats::{DocumentFormatError, PayloadType};
use search_types::error::{Code, ErrorCode, ResponseError};
use search_types::index_uid::{IndexUid, IndexUidFormatError};
use search_types::milli;
use search_types::milli::OrderBy;
use search_types::tasks::network::headers::{
...
pub enum MeilisearchHttpError {
    ...
    #[error(transparent)]
    HeedError(#[from] search_types::heed::Error),
    #[error(transparent)]
...
if_remote=if let Some(remote) = remote {
    ...
    UnprocessedNetworkTask { remote: Option<String>, task_uid: search_types::tasks::TaskId },
