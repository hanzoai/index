...
use search_types::tasks::network::headers::{GetHeader, SetHeader};
use search_types::tasks::network::{
    DbTaskNetwork, ImportData, ImportMetadata, Origin, TaskNetwork,
};
use search_types::tasks::{Task, TaskId};
    ...
    F: FnMut(&str, &Remote, &mut T),
{
    ...
    pub fn into_bytes(
        ...
        remote_name: &str,
        remote: &Remote,
    ) -> Result<Option<Bytes>, search_types::milli::Error> {
        Ok(match self {
            Body::NdJsonPayload(file) => {
                Some(Bytes::from_owner(unsafe { memmap2::Mmap::map(&file)? }))
            }
...
impl<'a> search_types::tasks::network::headers::GetHeader for ResponseWrapper<'a> {
