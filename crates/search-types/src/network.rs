...
pub mod route {
    ...
    pub fn network_control_path() -> PathAndQuery {
        // WARNING: if you change this path, you must also change the path in the network route macro in the search crate.
        PathAndQuery::from_static("/network/control")
    }

    pub fn multi_search_path() -> PathAndQuery {
        // WARNING: if you change this path, you must also change the path in the federated search route macro in the search crate.
