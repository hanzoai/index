...
use search_auth::{open_auth_store_env, AuthController};
use search_types::dynamic_search_rules::DynamicSearchRules;
use search_types::milli::constants::VERSION_MAJOR;
use search_types::milli::documents::{DocumentsBatchBuilder, DocumentsBatchReader};
use search_types::milli::progress::{EmbedderStats, Progress};
use search_types::milli::update::new::indexer;
use search_types::milli::update::{
...
use search_types::{compression, heed, milli, VERSION_FILE_NAME};
pub use option::Opt;
...
fn import_dump(
    ...
    if let Some(date) = dump_reader.date() {
        tracing::info!(
            version = ?dump_reader.version(), // TODO: get the search version instead of the dump version
            %date,
            "Importing a dump of search"
        );
    } else {
        tracing::info!(
            version = ?dump_reader.version(), // TODO: get the search version instead of the dump version
