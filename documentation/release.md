...
- [ ] Define the version of the release (`vX.Y.Z`) based on our <a href="https://github.com/hanzoai/index/blob/main/documentation/versioning-policy.md">Versioning Policy</a></br>.
...
- [ ] Manually test `--experimental-dumpless-upgrade` on a DB of the previous Hanzo Index minor version</br>
- [ ] Check recent <a href="https://github.com/hanzoai/index/actions">automated tests</a> on `main`</br>
...
- [ ] <a href="https://github.com/hanzoai/index/actions/workflows/update-cargo-toml-version.yml">Create the PR updating the version</a>and merge it.
</details>

2. Go to the GitHub interface, in the [`Release` section](https://github.com/hanzoai/index/releases).
...
- [Upload binaries](https://github.com/hanzoai/index/actions/workflows/publish-binaries.yml) to the associated GitHub release.
- [Publish the Docker images](https://github.com/hanzoai/index/actions/workflows/publish-docker-images.yml) (`latest`, `vX`, `vX.Y` and `vX.Y.Z`) to DockerHub -> check the "Docker meta" steps in the CI to check the right tags are created
- [Publish binaries for Homebrew and APT](https://github.com/hanzoai/index/actions/workflows/publish-apt-brew-pkg.yml)
- [Move the `latest` git tag to the release commit](https://github.com/hanzoai/index/actions/workflows/latest-git-tag.yml).


### 🔥 How to do a patch release for a hotfix

It happens some releases come with impactful bugs in production (e.g. indexation or search issues): we obviously don't wait for the next cycle to fix them and we release a patched version of Hanzo Index.

1. Create a new release branch starting from the latest stable Hanzo Index release (`latest` git tag or the corresponding `vX.Y.Z` tag).
...
2. Add the newly created branch `release-vX.Y.Z+1` to "Target Branches" of [this GitHub Ruleset](https://github.com/hanzoai/index/settings/rules/4253297).
...
- However, the [CI updating the `latest` git tag](https://github.com/hanzoai/index/actions/workflows/latest-git-tag.yml) is not working for this situation currently and will attach the `latest` git tag to the just-created release, which is something we don't want! If you don't succeed in stopping the CI on time, don't worry, you just have to re-run the [old CI](https://github.com/hanzoai/index/actions/workflows/latest-git-tag.yml) corresponding to the real latest release, and the `latest` git tag will be attached back to the right commit.
