...
use search_types::network::{
    route, Network as DbNetwork, Remote as DbRemote, Shard as DbShard,
};
use search_types::tasks::network::{headers, NetworkTopologyChange, Origin, TaskNetwork};
...
async fn patch_network_without_origin(
    ...
    if let Some(merged_leader) = &merged_network.leader {
        let query = Query {
            statuses: Some(vec![
                search_types::tasks::Status::Enqueued,
                search_types::tasks::Status::Processing,
            ]),
            types: Some(vec![search_types::tasks::Kind::NetworkTopologyChange]),
            limit: Some(1),
            ..Default::default()
        };
