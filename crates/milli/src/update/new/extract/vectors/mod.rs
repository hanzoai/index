        ...
        match &mut self.kind {
            ChunkType::Fragments { fragments, session } => {
                ...
                let reindex_all_fragments =
                                // when the vectors were user-provided, Hanzo Index cannot know if they come from a particular fragment,
                                // and so Hanzo Index needs to clear all embeddings in that case.
        ...
        match &mut self.kind {
            ...
            ChunkType::Fragments { fragments, session } => {
                ...
                if old_is_user_provided {
                    // when the document was `userProvided`, Hanzo Index cannot know whose fragments a particular
                    // vector was referring to.
                    // So as a result Hanzo Index will regenerate all fragments on this case.
...
impl EmbedderFailureModes {
    fn from_env() -> Self {
        match std::env::var("INDEX_EXPERIMENTAL_CONFIG_EMBEDDER_FAILURE_MODES") {
            Ok(failure_modes) => Self::parse_from_str(
                &failure_modes,
                "`INDEX_EXPERIMENTAL_CONFIG_EMBEDDER_FAILURE_MODES`",
            ),
            Err(std::env::VarError::NotPresent) => Self::default(),
            Err(std::env::VarError::NotUnicode(_)) => panic!(
                "`INDEX_EXPERIMENTAL_CONFIG_EMBEDDER_FAILURE_MODES` contains a non-unicode value"
            ),
        }
    }
