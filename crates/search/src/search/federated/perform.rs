...
use search_types::dynamic_search_rules::DynamicSearchRules;
use search_types::error::{Code, ResponseError};
use search_types::milli::order_by_map::OrderByMap;
use search_types::milli::progress::Progress;
use search_types::milli::score_details::{ScoreDetails, WeightedScoreValue};
use search_types::milli::vector::Embedding;
...
use search_types::settings::DEFAULT_PAGINATION_MAX_TOTAL_HITS;
...
impl PartitionedQueries {
    ...
    fn partition(
        ...
        if let Some(pagination_field) = federated_query.has_pagination() {
            return Err(HttpError::PaginationInFederatedQuery(
                query_index,
                pagination_field,
            )
            .into());
        ...
        if let Some(facets) = federated_query.has_facets() {
            ...
            return Err(HttpError::FacetsInFederatedQuery(
                query_index,
        ...
        if federated_query.has_personalize() {
            return Err(HttpError::PersonalizationInFederatedQuery(query_index).into());
        }

        if federated_query.has_remote_and_use_network() {
            return Err(HttpError::RemoteAndUseNetwork(query_index).into());
        }

        if federated_query.has_show_performance_details() {
            return Err(
                HttpError::ShowPerformanceDetailsInFederatedQuery(query_index).into()
            );
        }

        if federated_query.has_distinct() && federation.distinct.is_some() {
            return Err(
                HttpError::DistinctInFederatedQueryAndFederation(query_index).into()
            );
        }
                            ...
                            _ => {
                                ...
                                let Some(remote) = network.remotes.get(&remote_name) else {
                                    return Err(ResponseError::from_msg(
                                        ...
                                        search_types::error::Code::InvalidMultiSearchRemote,
                                    ));
                                };
                    ...
                    Ok(panic) => {
                        ...
                        remote_errors.insert(
                            ...
                            ResponseError::from_msg(
                                msg.to_string(),
                                search_types::error::Code::Internal,
                            ),
                        );
                    }
            ...
            let res: Result<(), ResponseError> = (|| {
                ...
                let hit_maker =
                    HitMaker::new(&index, &rtxn, format, formatter_builder).map_err(|e| {
                        HttpError::from_milli(e, Some(index_uid.to_string()))
                    })?;
                ...
                if let Some(facets_by_index) = facets_by_index {
                    ...
                    for facet in facets_by_index {
                        ...
                        if previous_facet_order != &index_facet_order {
                            return Err(HttpError::InconsistentFacetOrder {
                                facet: facet.clone(),
