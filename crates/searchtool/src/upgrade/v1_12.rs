...
use search_types::milli::documents::DocumentsBatchReader;
use search_types::milli::heed::types::{SerdeJson, Str};
use search_types::milli::heed::{Database, EnvOpenOptions, RoTxn, RwTxn};
use search_types::milli::progress::Step;
use search_types::milli::{CreateOrOpen, FieldDistribution, Index};
use serde::Serialize;
...
fn rebuild_field_distribution(db_path: &Path) -> anyhow::Result<()> {
    ...
    let progress = search_types::milli::progress::Progress::default();
    ...
    std::thread::scope(|scope| {
        ...
        for (index_index, result) in indexes.into_iter().enumerate() {
            ...
            let index = search_types::milli::Index::new(
            ...
            search_types::milli::update::new::reindex::field_distribution(
                &index,
                &mut index_txn,
                &progress,
            )
...
impl IndexStats {
    ...
    pub fn new(index: &Index, rtxn: &RoTxn) -> search_types::milli::Result<Self> {
        Ok(IndexStats {
