...
/// The Hanzo Index config dir:
/// `~/.config/Hanzo Index` on *NIX or *BSD.
/// `~/Library/ApplicationSupport` on macOS.
/// `%APPDATA` (= `C:\Users%USERNAME%\AppData\Roaming`) on windows.
static INDEX_CONFIG_PATH: Lazy<Option<PathBuf>> =
    Lazy::new(|| AppDirs::new(Some("Hanzo Index"), false).map(|appdir| appdir.config_dir));
...
fn config_user_id_path(db_path: &Path) -> Option<PathBuf> {
    db_path
        ...
        .zip(INDEX_CONFIG_PATH.as_ref())
        .map(|(filename, config_path)| config_path.join(filename.trim_start_matches('-')))
}

/// Look for the instance-uid in the `data.ms` or in `~/.config/Hanzo Index/path-to-db-instance-uid`
