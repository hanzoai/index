...
use search_types::deserr::{DeserrJsonError, DeserrQueryParamError};
use search_types::document_formats::{read_csv, read_json, read_ndjson, PayloadType};
use search_types::error::deserr_codes::*;
use search_types::error::{Code, ResponseError};
use search_types::heed::RoTxn;
use search_types::index_uid::IndexUid;
use search_types::milli::documents::sort::recursive_sort;
...
use search_types::milli::vector::parsed_vectors::ExplicitVectors;
use search_types::milli::{AscDesc, DocumentId, IndexFilter};
use search_types::serde_cs::vec::CS;
use search_types::star_or::OptionStarOrList;
use search_types::tasks::KindWithContent;
use search_types::{milli, Document, Index};
use mime::Mime;
...
#[routes::routes(
    ...
    tags(
        (
            name = "Documents",
            description = "Documents are objects composed of fields that can store any type of data. Each field contains an attribute and its associated value. Documents are stored inside [indexes](https://docs.hanzo.ai/docs/search).",
        ),
    ),
)]
...
/// Get document
///
/// Retrieve a single document by its [primary key](https://docs.hanzo.ai/docs/search) value.
#[routes::path(
        ...
        (status = 404, description = "Document not found.", body = ResponseError, content_type = "application/json", example = json!(
            {
              ...
              "type": "invalid_request",
              "link": "https://docs.hanzo.ai/docs/errors#document_not_found"
            }
        )),
...
/// Delete document
///
/// Delete a single document by its [primary key](https://docs.hanzo.ai/docs/search).
#[routes::path(
...
pub struct BrowseQuery {
    ...
    /// matching [primary key](https://docs.hanzo.ai/docs/search) values will be returned. If not specified, all
...
pub struct UpdateDocumentsQuery {
    /// The [primary key](https://docs.hanzo.ai/docs/search) field for uniquely identifying each document.
...
///
/// Delete multiple documents in one request by providing an array of [primary key](https://docs.hanzo.ai/docs/search) values.
#[routes::path(
    security(("Bearer" = ["documents.delete", "documents.*", "*"])),
    params(
        ("index_uid" = String, example = "movies", description = "Unique identifier of the index.", nullable = false),
...
///
/// Use a [RHAI function](https://rhai.rs/book/engine/hello-world.html) to edit one or more documents directly in Hanzo Index. The function receives each document and returns the modified document.
///
