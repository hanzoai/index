...
use search_types::locales::{Locale, LocalizedAttributesRuleView};
use search_types::milli::update::Setting;
use search_types::milli::{FilterableAttributesRule, ForeignKey};
    ...
    fn aggregate(self: Box<Self>, new: Box<Self>) -> Box<Self> {
        Box::new(Self {
            ranking_rules: RankingRulesAnalytics {
                ...
                word_position_position: new
                    .ranking_rules
                    .word_position_position
                    .or(self.ranking_rules.word_position_position),
...
pub struct RankingRulesAnalytics {
    ...
    pub word_position_position: Option<usize>,
    ...
    pub fn new(rr: Option<&Vec<RankingRuleView>>) -> Self {
        RankingRulesAnalytics {
            words_position: rr.as_ref().and_then(|rr| {
                rr.iter()
                    .position(|s| matches!(s, search_types::settings::RankingRuleView::Words))
            }),
            typo_position: rr.as_ref().and_then(|rr| {
                rr.iter()
                    .position(|s| matches!(s, search_types::settings::RankingRuleView::Typo))
            }),
            proximity_position: rr.as_ref().and_then(|rr| {
                rr.iter().position(|s| {
                    matches!(s, search_types::settings::RankingRuleView::Proximity)
                })
            }),
            attribute_position: rr.as_ref().and_then(|rr| {
                rr.iter().position(|s| {
                    matches!(s, search_types::settings::RankingRuleView::Attribute)
                })
            }),
            attribute_rank_position: rr.as_ref().and_then(|rr| {
                rr.iter().position(|s| {
                    matches!(s, search_types::settings::RankingRuleView::AttributeRank)
                })
            }),
            word_position_position: rr.as_ref().and_then(|rr| {
                rr.iter().position(|s| {
                    matches!(s, search_types::settings::RankingRuleView::WordPosition)
                })
            }),
            sort_position: rr.as_ref().and_then(|rr| {
                rr.iter()
                    .position(|s| matches!(s, search_types::settings::RankingRuleView::Sort))
            }),
            exactness_position: rr.as_ref().and_then(|rr| {
                rr.iter().position(|s| {
                    matches!(s, search_types::settings::RankingRuleView::Exactness)
                })
            }),
            values: rr.as_ref().map(|rr| {
                rr.iter()
                    .filter(|s| {
                        matches!(
                            s,
                            search_types::settings::RankingRuleView::Asc(_)
                                | search_types::settings::RankingRuleView::Desc(_)
                        )
                    })
                    .map(|x| x.to_string())
                    .collect::<Vec<_>>()
