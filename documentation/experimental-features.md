...
An experimental feature is a feature present in the final Hanzo Index binary that is not considered stable. This means the API might become incompatible between two Hanzo Index releases.
...
Since the release of v1, Hanzo Index is considered a stable binary and its API cannot break between minor and patch versions. This means it is impossible to make breaking changes to a feature without releasing a major version.

This limitation, which guarantees our users Hanzo Index is a stable and reliable product, also applies to new features. If we introduce a new feature in one release, any breaking changes will require a new major release.
...
- The API and behavior of an experimental feature can break between two minor versions of Hanzo Index
- The experimental feature process described here can significantly change between 2 minor versions of Hanzo Index
...
- Hanzo Index CLI: update the `--help` command in the Hanzo Index binary so it redirects users to the related GitHub discussion and warns them about the unstable state of the features
...
- API can break between 2 versions of Hanzo Index. People using the experimental feature in production should pay extra attention to it.
...
### Why does Hanzo Index need to be restarted when activating an experimental feature?

Hanzo Index uses LMDB to store both documents and internal application data, such as Hanzo Index tasks. Altering these internal data structures requires closing and re-opening the LMDB environment.

If an experimental feature implementation involves a modification of internal data structures, users must restart Hanzo Index. This cannot be done via HTTP routes.
...
Under some circumstances, Hanzo Index might have issues when reading a database generated by a different Hanzo Index release. This might cause an instance to crash or work with faulty data.

This is already a possibility when migrating between minor Hanzo Index versions, and is more likely to happen when activating a new experimental feature. The opposite operation—migrating a database with experimental features activated to a database where those features are not active—is currently riskier. As we develop and improve the development of experimental features, this procedure will become safer and more reliable.

### Restarting Hanzo Index and migrating databases with dumps to activate an experimental feature is inconvenient. Will this improve in the future?
