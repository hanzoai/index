...
There is no JavaScript SDK; the API is JSON over HTTP, so `fetch` is the client.
...
// Index documents
await fetch(`${BASE}/indexes/products/documents`, {
  method: 'POST',
  headers,
  body: JSON.stringify([
    { id: 1, title: 'Hanzo Agent SDK', category: 'AI' },
...
// Search
const res = await fetch(`${BASE}/indexes/products/search`, {
  method: 'POST',
  headers,
  body: JSON.stringify({ q: 'agent', limit: 10, attributesToHighlight: ['title'] }),
})
console.log((await res.json()).hits)
```
...
Documents are JSON objects. Hanzo Index accepts arrays, NDJSON, and CSV formats. The maximum payload size defaults to 100 MB.
...
Hanzo Index includes an OpenAI-compatible chat completions endpoint. The LLM automatically searches your indexes to answer questions, with streaming SSE responses:
