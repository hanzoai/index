...
impl Stream for Payload {
    type Item = Result<web::Bytes, HttpError>;
        ...
        match Pin::new(&mut self.payload).poll_next(cx) {
            Poll::Ready(Some(result)) => match result {
                Ok(bytes) => match self.remaining.checked_sub(bytes.len()) {
                    ...
                    None => {
                        Poll::Ready(Some(Err(HttpError::PayloadTooLarge(self.limit))))
                    }
                },
                x => Poll::Ready(Some(x.map_err(HttpError::from))),
            },
            otherwise => otherwise.map(|o| o.map(|o| o.map_err(HttpError::from))),
        }
    }
}
