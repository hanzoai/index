...
macro_rules! test_setting_routes {
    ($({setting: $setting:ident, update_verb: $update_verb:ident, default_value: $default_value:tt},) *) => {
        ...
        async fn all_setting_tested() {
            let expected = std::collections::BTreeSet::from_iter(search::routes::indexes::settings::ALL_SETTINGS_NAMES.iter());
...
async fn secrets_are_hidden_in_settings() {
    ...
    let (response, code) = index
        ...
        .await;
    search_snap::snapshot!(code, @"202 Accepted");

    search_snap::snapshot!(search_snap::json_string!(response, { ".taskUid" => "[task_uid]", ".duration" => "[duration]", ".enqueuedAt" => "[date]", ".startedAt" => "[date]", ".finishedAt" => "[date]" }),
    @r###"
    {
      "taskUid": "[task_uid]",
      "indexUid": "[uuid]",
    ...
    search_snap::snapshot!(code, @"200 OK");
    search_snap::snapshot!(search_snap::json_string!(response["details"]), @r#"
    {
      "embedders": {
        "default": {
          "source": "rest",
