...
/// Deserialize the `content` field that may arrive as a plain string (OpenAI)
/// or as an array of typed parts (Mistral). Mistral can return chunks like:
/// `[{"type":"reference","reference_ids":[]},{"type":"text","text":"..."}]`
/// We extract and concatenate only the `text` parts to stay compatible with
/// the `Option<String>` representation used everywhere else.
fn deserialize_content_string_or_parts<'de, D>(deserializer: D) -> Result<Option<String>, D::Error>
where
    ...
    match v {
        ...
        serde_json::Value::Array(items) => {
            ...
            for item in items {
                ...
                let Some(serde_json::Value::String(ty)) = obj.get("type") else {
                    continue;
                };
                if ty == "text" {
...
/// A developer message in the conversation.
...
/// A content part in a system message (text only).
#[derive(Debug, Serialize, Deserialize, Clone, PartialEq, ToSchema)]
#[serde(tag = "type")]
#[serde(rename_all = "snake_case")]
...
/// A content part in a tool message (text only).
#[derive(Debug, Serialize, Deserialize, Clone, PartialEq, ToSchema)]
#[serde(tag = "type")]
#[serde(rename_all = "snake_case")]
...
/// An assistant message in the conversation, possibly including tool calls.
#[derive(Debug, Serialize, Deserialize, Default, Clone, Builder, PartialEq, ToSchema)]
...
/// Deprecated: A function response message. Use tool messages instead.
#[derive(Debug, Serialize, Deserialize, Default, Clone, Builder, PartialEq, ToSchema)]
...
pub struct ChatCompletionResponseMessage {
    ...
    #[serde(default, deserialize_with = "deserialize_content_string_or_parts")]
    pub content: Option<String>,
    /// The refusal message generated by the model.
    pub refusal: Option<String>,
...
/// A streamed function call (partial name and arguments).
#[derive(Debug, Deserialize, Serialize, Clone, PartialEq, ToSchema)]
...
/// A streamed tool call chunk (partial tool call data).
#[derive(Debug, Deserialize, Serialize, Clone, PartialEq, ToSchema)]
...
mod tests {
    ...
    #[test]
    fn openai_stream_delta_content_string_deserialize() {
        let json = r#"{
          "id":"x",
          "object":"chat.completion.chunk",
          "created":1,
    ...
    #[test]
    fn stream_delta_null_content_deserialize() {
        let json = r#"{
          "id":"x",
          "object":"chat.completion.chunk",
          "created":1,
    ...
    #[test]
    fn mistral_reference_only_content_parts_deserialize() {
        // When Mistral returns only reference parts with no text, content should be None.
        let json = r#"{
          "id":"x",
          "object":"chat.completion.chunk",
          "created":1,
